...
static void Log_Write_Raw()
{
	...
	//sends raw accelerations and angular rates. I don't think we need this.
 ...
 static void Log_Write_Fhp()
 {
	...
	DataFlash.WriteLong(long(100000*vscl_fhp.fhp_access(0)));
	DataFlash.WriteLong(long(100000*vscl_fhp.fhp_access(1)));
	DataFlash.WriteLong(long(100000*vscl_fhp.fhp_access(2)));
	DataFlash.WriteLong(long(100000*vscl_fhp.fhp_access(3)));
	//write the temperature value
//what should be the precision used here?
	DataFlash.WriteInt(int(100*vscl_fhp.fhp_temp_humid(0)));
	//write the humdity
	DataFlash.WriteInt(int(100*vscl_fhp.fhp_temp_humid(1)));
	//write the end byte:
	DataFlash.WriteByte(END_BYTE);

	//send psensor1,psensor2,psensor3 values over the wind message. These values are sufficient to get speed, alpha, and beta
...
static void Log_Read_Fhp()
{
	long pres[4];
	//read values
	pres[0] = DataFlash.ReadLong();
	pres[1] = DataFlash.ReadLong();
	pres[2] = DataFlash.ReadLong();
	pres[3] = DataFlash.ReadLong();
	int temp = DataFlash.ReadInt();
	int humid = DataFlash.ReadInt();
	//print to serial
	Serial.printf_P(PSTR("FHP: %6.6f, %6.6f, %6.6f, %6.6f, %6.6f, %6.6f\n"),float(pres[0]*1e-5),float(pres[1]*1e-5),float(pres[2]*1e-5),float(pres[3]*1e-5),float(temp)/100.0,float(humid)/100.0);
}

 //VSCL: write five hole probe data to flash memory
 static void Log_Write_Ctrl()
 {
	//write checksum headers
	DataFlash.WriteByte(HEAD_BYTE1);
	DataFlash.WriteByte(HEAD_BYTE2);
	//write message identifier
	DataFlash.WriteByte(LOG_CTRL_MSG);
//write elevator
	DataFlash.WriteInt(g.channel_pitch.radio_out);
//write throttle
	DataFlash.WriteInt(g.channel_throttle.radio_out);
//write aileron
	DataFlash.WriteInt(g.channel_roll.radio_out);
//write rudder
	DataFlash.WriteInt(g.channel_rudder.radio_out);
	//write the end byte:
	DataFlash.WriteByte(END_BYTE);
	
	//write an RC_CHANNELS_SCALED message with RC channels 1 through 8 written as 0-100%
	Serial.printf("\n%i,%i,%i,%i,%i,%i,%i,%i\n",
          HEAD_BYTE1,
          HEAD_BYTE2,
          LOG_CTRL_MSG,
...
static void Log_Read_Ctrl()
{
        int ctrl_vals[4];
	//read values
	ctrl_vals[0] = DataFlash.ReadInt();
	ctrl_vals[1] = DataFlash.ReadInt();
	ctrl_vals[2] = DataFlash.ReadInt();
	ctrl_vals[3] = DataFlash.ReadInt();
	//print to serial
	Serial.printf_P(PSTR("CTRL: %d, %d, %d, %d,\n"),ctrl_vals[0],ctrl_vals[1],ctrl_vals[2],ctrl_vals[3]);
}
                        ...
                        while (page < end_page && page != -1) {
                            ...
                            case 2:
                                if(data == LOG_ATTITUDE_MSG) {
									...
									Log_Read_Fhp();//vscl read five hole probe data
									log_step++;
                                } else if (data == LOG_CTRL_MSG) {
                                    Log_Read_Ctrl();
                                    log_step++;
                                }else {
                                    if(data == LOG_GPS_MSG) {
